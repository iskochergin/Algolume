...
(function () {
    ...
    const setIcon = () => {
        icon.innerHTML = root.classList.contains('light-theme')
            ? '<path d="M21.752 14.002A9 9 0 0 1 9.998 2.248 7 7 0 1 0 21.752 14z"/>'  // moon
    ...
    // Apply the chosen theme to all CodeMirror instances on initial load
    applyCMTheme();
})();

// Make sure the example tab is visible and active on load.
...
document.addEventListener("DOMContentLoaded", function () {
    // Initialize CodeMirror for example code
    ...
    // Initialize CodeMirror for user code
    ...
    if (exampleRunBtn && window.exampleCM) {
        exampleRunBtn.addEventListener('click', async e => {
            ...
            try {
                const res = await fetch(`${API_BASE}/new-debug-page-zfunction`, {
                    method: 'POST',
    ...
    if (userRunBtn && window.userCM) {
        userRunBtn.addEventListener('click', async e => {
            ...
            if (!code.includes(zVar)) {
                return CustomAlert(`Variable "${zVar}" not found in your code.`);
            }
            if (code.split('\n').length > 300) {
                return CustomAlert('Code is too long! Max 300 lines.');
            }
