...
document.addEventListener("DOMContentLoaded", function () {
    ...
    ['theory-code-snippet-1', 'theory-code-snippet-2', 'theory-code-snippet-3', 'theory-code-snippet-4']
        .forEach((id, idx) => {
            const el = document.getElementById(id);
            if (el) {
                window['theoryCM' + (idx + 1)] = CodeMirror.fromTextArea(el, {
                    lineNumbers: true,
                    mode: "python",
                    theme: document.body.classList.contains('light-theme') ? CM_LIGHT : CM_DARK,
                    indentUnit: 4,
                ...
                window['theoryCM' + (idx + 1)].getWrapperElement().classList.add('theory-cm');
            }
        });
    ...
    if (userRunBtn && window.userCM) {
        userRunBtn.addEventListener('click', async event => {
            ...
            if (!code.includes(dpVar) || !code.includes(parentVar)) {
                CustomAlert(`Переменная "${!code.includes(dpVar) ? dpVar : parentVar}" не найдена в вашем коде.`);
                return;
            }
            if (code.split('\n').length > 300) {
                CustomAlert('The code is too large! The maximum number of lines is 300.');
    ...
    // Refresh CodeMirror when sidebar transitions end.
    const contentContainer = document.getElementById('content-container');
    if (contentContainer) {
